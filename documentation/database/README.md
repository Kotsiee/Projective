...
numbering ranges (0000–0499), the `pgvector`/Triggered Search Table pattern, the storage
quarantine lifecycle, Edge Function/webhook standards, and general RLS principles.
...
[Schemas.md](Schemas.md) is the top-level ERD-adjacent reference: the 11 `CREATE SCHEMA`
statements and every custom enum type with its literal values.
...
✅ = populated with real schema detail. `—` = stub file stamped `_Not yet documented._` — this is
an intentional placeholder, not a deletion or accident. Every `Functions.md` across all 11 domains
is currently a stub; populate them as RPCs are implemented.

## For Future Agents

- Do not delete a stub file because it looks empty — it marks a domain that's scaffolded but not
  yet written. Replace the `_Not yet documented._` stub content when you add real schema detail.
