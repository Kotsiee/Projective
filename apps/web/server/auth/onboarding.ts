...
export async function onboarding(
	...
	try {
		...
		// FIX: Switched from getSession() to getUser()
		// getSession() is null in stateless/SSR mode. getUser() validates the token via API.
		...
		const { error: userError } = await supabase.schema('org').rpc('onboard_user', {
