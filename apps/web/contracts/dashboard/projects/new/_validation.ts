...
const StageOpenSeatSchema = z.object({
	description_of_need: z.string().min(10, 'Please describe the need in detail').max(500),
...
export const StageSchema = z.object({
	...
	file_duration_mode: z.enum(['fixed_deadline', 'relative_duration', 'no_due_date']).optional(),
	...
	if (data.start_trigger_type === StartTriggerType.FixedDate && !data.fixed_start_date) {
		return false;
	}
	return true;
}, {
...
export const CreateProjectSchema = z.object({
	...
	industry_category_id: z.string().uuid({ message: 'Please select an industry category' }),

	// Settings
	visibility: z.nativeEnum(Visibility),
	currency: z.string().length(3).regex(/^[A-Z]{3}$/, 'Must be a 3-letter currency code'),
