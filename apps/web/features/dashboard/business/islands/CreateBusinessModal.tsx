/**
 * @file CreateBusinessModal.tsx
 * @description Low-friction business creation dialog. Only a display Name and a unique
 * `@handle` are required — the entity is created instantly as a Draft/Unverified record
 * and all extended metadata (branding, legal, billing) is deferred to the locked settings
 * page. Islands are "dumb": this posts to `/api/v1/dashboard/business` and never touches
 * the DB directly.
 */
...
import { IconBuilding } from '@tabler/icons-preact';
...
export default function CreateBusinessModal({ isOpen, onClose }: CreateBusinessModalProps) {
	...
	const onNameChange = (v: string) => {
		name.value = v;
		// Mirror the name into the handle until the user takes control of it.
	...
	const submit = async () => {
		...
		if (name.value.trim().length < 2) {
			error.value = 'Give your business a name (at least 2 characters).';
			return;
		}
		...
		const request = async () => {
			...
			if (!res.ok) {
				throw new Error(data?.error?.message || data?.error || 'Could not create business');
			}
			// Head straight to the (locked) settings page to finish the draft.
			setTimeout(() => {
				globalThis.location.href = data.redirectTo ??
					`/business/${data.slug ?? slug.value}/settings`;
			}, 600);
			return 'Business drafted — let’s finish setup.';
		};
			...
			>
				...
				>
					<p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>
						Reserve your space now — you can add branding, legal and billing details later. It
						starts as a <strong>Draft</strong> until verified.
					</p>
