...
export function ProjectStage({ id }: { id: number }) {
	...
	const stageTypes: SelectOption<string>[] = [
		...
		{
			label: "Session Based (Call/Workshop)",
			value: StageType.SessionBased,
		},
		{
			label: "Management Based (Contractual)",
			value: StageType.ManagementBased,
		},
		{
			label: "Maintenance Based (Recurring)",
			value: StageType.MaintenanceBased,
		},
	];
	...
	const startTriggers: SelectOption<string>[] = [
		...
		{
			label: "Dependent on Stage",
			value: StartTriggerType.DependentOnStage,
		},
	];
		...
		<div className="new-project__stage">
			...
			<div className="stage-section">
				...
				<SelectField
					...
					onChange={(v) =>
						updateStage("start_trigger_type", v as string)}
					searchable={false}
					multiple={false}
					floating
					required
				...
				<div
					className="type-specific-config"
