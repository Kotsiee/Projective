...
export function useSplitter({
	...
	const calculateMove = useCallback(
		(index: number, deltaPercent: number, baseSizes: number[]) => {
			const leftIdx = index;
	...
	const toggleCollapse = useCallback((index: number) => {
		...
		if (currentSize > 0.5) {
			...
			let restoreSize = collapsedState.current.get(index) ||
				config.minSize || minPaneSize || 10;
			const neighborSize = currentSizes[neighborIdx];
			const neighborMin = configsRef.current[neighborIdx]?.minSize ??
				minPaneSize;
