...
BEGIN
  ...
  INSERT INTO org.business_profiles (
    owner_user_id, name, slug, headline, description, logo_file_id, legal_name, billing_email, country, address_line_1, address_city, address_zip, tax_id, default_currency
  ) VALUES (
    v_user_id, payload->>'name', v_slug, COALESCE(payload->>'headline', ''), COALESCE(payload->'description', '{}'::jsonb), (payload->>'logo_file_id')::uuid, payload->>'legal_name', payload->>'billing_email', payload->>'country', payload->>'address_line_1', payload->>'address_city', payload->>'address_zip', payload->>'tax_id', COALESCE(payload->>'default_currency', 'USD')
  ...
  INSERT INTO security.session_context (user_id, active_profile_type, active_profile_id, updated_at) VALUES (v_user_id, 'business', v_new_business_id, NOW()) ON CONFLICT (user_id) DO UPDATE SET active_profile_type = 'business', active_profile_id = v_new_business_id, updated_at = NOW();
